...
|:--:|:--:|:--:|---|---|---|
| **D1** | 09-18 五 | **总纲 + L4a 内核** | 六层栈逐层讲 + "一条消息的一生" + 手写 ReAct | 六层栈总纲表（先填 L4 行）+ 生命周期总图 v1 + 五范式对比表 | `00-我的深度整合专题\循环工程深度解析\Loop_Engineering_Deep_Dive.md`（新建）|
| **D2** | 09-19 六 | **L2 上下文** | "凭什么决定记什么/忘什么" + "向量库不是 RAG" | 记忆对照表（含 mem0 列）+ RAG 十步管线卡 + RAG/记忆/长上下文边界卡 | `P2-记忆实现对照提纯.md`（待写）+ 已有两份专题 |
| **D3** | 09-20 日 | **L1 推理 + L3 动作** | 模型怎么选/怎么跑/怎么路由 + 工具协议与权限 | L1 选型卡 + 工具协议对照表 + 权限决策树（+ 写码挑战③） | `推理与路由深度解析`（待写）|
| **D4** | 09-21 一 | **L4b 选型 + 协同** | "LangGraph 和 Hermes 是同层二选一" + 多智能体 vs 单 agent | 框架选型矩阵终版 + runtime 五方对照表（LangGraph/Hermes/waku/pi/DSH） | `运行时选型-LangGraph-vs-Hermes.md`（新建）+ `P4` |
| **D5** | 09-22 二 | **L5 观测 + L6 界面 + 收口** | agent 怎么评测/怎么进 CI + 六层栈总演练 | **面试总纲 v1** + 模拟面试记录 + CICD 背题卡 | `P5-评测CICD概念提纯.md`（待写）|
